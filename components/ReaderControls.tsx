...
const ReaderControls: React.FC<ReaderControlsProps> = ({ preferences, setPreferences }) => {
    ...
    const themeOptions: { key: ReaderTheme; name: string; bg: string; border: string; ring: string }[] = [
        { key: 'ivory_classic', name: 'Sáng', bg: 'bg-[#F8F4EC]', border: 'border-[#3A2F2A]/20', ring: 'ring-[#3A2F2A]/40' },
        { key: 'soft_mint', name: 'Mint', bg: 'bg-[#EAF7F2]', border: 'border-[#263636]/20', ring: 'ring-[#263636]/40' },
        { key: 'peach_dust', name: 'Đào', bg: 'bg-[#FFF1E6]', border: 'border-[#4A3F3A]/20', ring: 'ring-[#4A3F3A]/40' },
        { key: 'light_lavender', name: 'Tím', bg: 'bg-[#F3F0FA]', border: 'border-[#2E2A3B]/20', ring: 'ring-[#2E2A3B]/40' },
        { key: 'deep_blue_night', name: 'Đêm', bg: 'bg-[#0F172A]', border: 'border-[#E2E8F0]/20', ring: 'ring-[#E2E8F0]/40' },
        { key: 'forest_dark', name: 'Rừng', bg: 'bg-[#0F1F1C]', border: 'border-[#F1EDE2]/20', ring: 'ring-[#F1EDE2]/40' },
    ];
                        ...
                        <div className={`${popoverClass} grid grid-cols-3 gap-2 w-[180px]`}>
                            {themeOptions.map(t => (
                                <button key={t.key} onClick={() => handleThemeChange(t.key)}
                                    className={`relative aspect-square rounded-xl border-2 transition-all duration-200 hover:scale-105 focus:outline-none ${t.bg} ${preferences.theme === t.key ? `${t.border} ${t.ring} ring-2 ring-offset-2 dark:ring-offset-stone-900 z-10 scale-105` : 'border-transparent hover:border-slate-300/50 shadow-sm'
                                        }`} title={t.name}
                                >
