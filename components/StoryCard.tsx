  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[2/3] overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-black/70 via-black/30 to-transparent pointer-events-none opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300"></div>
        ...
        {/* Badge View (Góc trái dưới) */}
        <div className="absolute bottom-2 left-2 flex items-center gap-1 text-white text-xs font-semibold drop-shadow-md opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300">
          <EyeIcon className="w-3.5 h-3.5 opacity-90" />
          <span>{formatNumber(views)}</span> 
        </div>

        {/* Badge: Rating (Góc phải dưới) */}
        <div className="absolute bottom-2 right-2 flex items-center gap-1 text-yellow-400 text-xs font-bold drop-shadow-md opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300">
          <span>{avgRating}</span>
      ...
      <div className="flex flex-col flex-1 p-3">
        <h3 className="font-bold text-sukem-text text-base leading-snug line-clamp-2 mb-2 group-hover:text-sukem-primary transition-colors duration-200 min-h-[2.5rem]" title={story.title}>
          {story.title}
        </h3>
