...
import {
    ...
    EyeSlashIcon,
    XMarkIcon,
    MinusIcon // <--- Thêm icon cho dấu phân cách
...
const CustomEditor: React.FC<CustomEditorProps> = ({ value, onChange }) => { //
    ...
    // --- START: Hàm chèn dấu phân cách ---
    const handleInsertSeparator = () => {
        const separatorHtml = `
            <div style="text-align: center; margin: 1.5em 0; display: flex; align-items: center; gap: 1em; opacity: 0.7; color: currentColor;">
                <hr style="flex-grow: 1; border-top: 1px solid currentColor;">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="display: inline-block;"><path d="M12 2L22 12L12 22L2 12L12 2Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                <hr style="flex-grow: 1; border-top: 1px solid currentColor;">
            </div>
            <p><br></p> `;
        execCmd('insertHTML', separatorHtml);
    };
    // --- END: Hàm chèn dấu phân cách ---
        ...
        <>
            <div className="flex flex-wrap items-center gap-1 p-2"> {/* */}
                ...
                <label className="relative flex items-center cursor-pointer" onMouseDown={handleToolbarMouseDown}> {/* */}
                    ...
                    <input type="color" onChange={(e) => execCmd('foreColor', e.target.value)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" aria-label="Màu chữ"/> {/* */}
                </label>
                <div className="w-px h-6 bg-slate-300 dark:bg-slate-600 mx-1"></div> {/* */}
                ...
                <ToolbarButton onClick={handleInsertFrame} ariaLabel="Tạo khung văn bản"><CodeBracketSquareIcon className="w-5 h-5" /></ToolbarButton> {/* */}
                {/* --- START: Nút chèn dấu phân cách --- */}
                <ToolbarButton onClick={handleInsertSeparator} ariaLabel="Chèn dấu phân cách">
                    <MinusIcon className="w-5 h-5" /> {/* Hoặc chọn icon khác phù hợp */}
                </ToolbarButton>
                {/* --- END: Nút chèn dấu phân cách --- */}
                <div className="w-px h-6 bg-slate-300 dark:bg-slate-600 mx-1"></div> {/* */}
            ...
            {showFind && ( //
                 <div className="flex flex-col sm:flex-row gap-2 p-2 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50"> {/* */}
                    ...
                    <button onClick={handleReplaceAll} className="px-3 py-2 bg-orange-500 text-white font-semibold rounded hover:bg-orange-600 text-sm whitespace-nowrap disabled:opacity-50" disabled={!findText}>Thay thế tất cả</button> {/* */}
                     <ToolbarButton onClick={() => setShowFind(false)} ariaLabel="Đóng tìm kiếm"><XMarkIcon className="w-5 h-5" /></ToolbarButton> {/* */}
                </div>
            )}
        </>
    );
