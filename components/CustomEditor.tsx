...
const CustomEditor: React.FC<CustomEditorProps> = ({ value, onChange }) => {
    ...
    const [isToolbarCollapsed, setIsToolbarCollapsed] = useState(() => {
        const saved = localStorage.getItem('editorToolbarCollapsed');
        return saved === 'true';
    });
    ...
    // Lưu trạng thái toolbar vào localStorage khi thay đổi
    useEffect(() => {
        localStorage.setItem('editorToolbarCollapsed', isToolbarCollapsed.toString());
    }, [isToolbarCollapsed]);
    ...
    return (
        <div ref={containerRef} className="border border-slate-300 dark:border-slate-600 rounded-md overflow-visible bg-white dark:bg-slate-800 shadow-sm relative">
            {/* Toolbar Sticky: Sử dụng position: sticky để thanh công cụ tự đi theo khi cuộn */}
            <div className="sticky top-0 z-[100] bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md shadow-lg mb-0">
                {/* Nút toggle chỉ hiện trên mobile */}
                <button
                    type="button"
                    onClick={() => setIsToolbarCollapsed(!isToolbarCollapsed)}
                    className="md:hidden w-full flex items-center justify-center gap-2 p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                    aria-label={isToolbarCollapsed ? "Mở rộng thanh công cụ" : "Thu gọn thanh công cụ"}
                >
                    {isToolbarCollapsed ? (
                        <>
                            <ChevronDownIcon className="w-4 h-4" />
                            <span className="text-xs font-medium">Hiện thanh công cụ</span>
                        </>
                    ) : (
                        <>
                            <ChevronUpIcon className="w-4 h-4" />
                            <span className="text-xs font-medium">Ẩn thanh công cụ</span>
                        </>
                    )}
                </button>

                {/* Toolbar content - ẩn trên mobile khi collapsed */}
