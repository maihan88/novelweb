...
const HeroBanner: React.FC<HeroBannerProps> = ({ interval = 7000 }) => {
  ...
  return (
    ...
    <div className="relative h-[65vh] md:h-[70vh] lg:h-[75vh] max-h-[500px] md:max-h-[600px] lg:max-h-[750px] min-h-[400px] w-full overflow-hidden rounded-2xl group shadow-lg border border-sukem-border bg-sukem-card">
      ...
      <div className="relative z-20 h-full w-full flex flex-col items-center justify-end gap-3 text-center p-6 pb-14 sm:p-8 sm:pb-16 lg:pb-24 lg:gap-5 transition-all">
        <div className="animate-fade-in max-w-3xl w-full flex flex-col items-center">
          ...
          >
            <h1
              className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold font-serif tracking-tight leading-tight py-2 px-4 select-none transition-all"
              style={gradientStyle}
            >
              {currentStory.title}
            </h1>
          ...
          {/* Description */}
          <p className="mt-2 lg:mt-4 text-xs sm:text-sm lg:text-base text-white/90 max-w-lg lg:max-w-2xl mx-auto line-clamp-2 leading-relaxed font-medium select-none transition-all">
          ...
          {/* Tags */}
          <div className="flex flex-wrap gap-1.5 mt-3 lg:mt-5 justify-center max-w-xl select-none">
            {currentStory.tags && currentStory.tags.slice(0, 5).map(tag => (
              <span key={tag} className="text-[10px] sm:text-xs lg:text-sm font-medium border border-white/20 bg-black/30 text-white/90 px-2.5 lg:px-3 py-0.5 lg:py-1 rounded-full">
                {tag}
              </span>
            ))}
          </div>
      ...
      {/* Dots Navigation */}
      <div className="absolute bottom-4 lg:bottom-6 left-1/2 -translate-x-1/2 flex space-x-1.5 lg:space-x-2 z-30">
        {stories.map((_, index) => (
          <button
            key={index}
            onClick={() => goToSlide(index)}
            className={`h-1 lg:h-1.5 rounded-full transition-all duration-300 ${currentIndex === index ? 'w-5 lg:w-8' : 'bg-white/40 w-1 lg:w-1.5 hover:bg-white/80'}`}
