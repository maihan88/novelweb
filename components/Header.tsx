...
const Header: React.FC = () => {
    ...
    <> 
      ...
      {currentUser?.role === 'admin' && ( 
        <div className="relative group/admin py-2">
           ...
           <div className="absolute top-full left-0 mt-0 w-56 bg-sukem-card border border-sukem-border shadow-xl rounded-xl opacity-0 invisible group-hover/admin:opacity-100 group-hover/admin:visible transition-all duration-200 z-50 overflow-hidden transform origin-top-left group-hover/admin:scale-100 scale-95">
  ...
  return (
    <header className="relative w-full z-40 bg-sukem-card border-b border-sukem-border shadow-sm transition-colors duration-300">
      ...
      <div className={`md:hidden absolute top-full left-0 w-full bg-sukem-card border-b border-sukem-border shadow-xl transition-all duration-300 ease-in-out overflow-hidden z-50 ${isMenuOpen ? 'max-h-[85vh] opacity-100 overflow-y-auto' : 'max-h-0 opacity-0'}`}>
