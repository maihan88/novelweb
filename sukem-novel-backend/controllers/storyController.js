    ...
    try {
        ...
        if (!useAdvancedFilter) {
            ...
            const storiesWithLatest = await Promise.all(storiesRaw.map(async (story) => {
                const latestChapter = await Chapter.findOne({ storyId: story._id, isRaw: false })
                    .select('title createdAt id volumeId').sort({ createdAt: -1 });
                let volumeName = null;
                if (latestChapter && latestChapter.volumeId && story.volumes) {
                    const vol = story.volumes.find(v => v.id === latestChapter.volumeId);
                    if (vol) volumeName = vol.title;
                }
                const count = await Chapter.countDocuments({ storyId: story._id });
                return { ...story, latestChapter: latestChapter ? { title: latestChapter.title, createdAt: latestChapter.createdAt, id: latestChapter.id, volumeName } : null, chapterCount: count };
            }));
        ...
        // --- LUỒNG 2: NÂNG CAO (Có lọc chương) ---
            ...
            const storiesWithLatest = await Promise.all(storiesRaw.map(async (story) => {
                ...
                return { ...story, latestChapter: latestChapter ? { title: latestChapter.title, createdAt: latestChapter.createdAt, id: latestChapter.id, volumeName } : null, chapterCount: story.calculatedChapterCount };
            }));
...
// @desc    Lưu đánh giá từ user (rating)
