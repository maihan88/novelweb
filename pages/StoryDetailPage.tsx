...
const StoryDetailPage: React.FC = () => {
    ...
    const handleRating = async (rating: number) => {
        if (!currentUser || !storyId) { showToast("Bạn cần đăng nhập để đánh giá.", "warning"); return; }
        if (userRating !== undefined) { showToast("Bạn đã đánh giá truyện này rồi.", "info"); return; }
        ...
        try { await addRatingToStory(storyId, rating); showToast("Đánh giá thành công!", "success"); }
        catch (e) { showToast('Có lỗi xảy ra khi gửi đánh giá.', 'error'); fetchStory(); }
    };
        ...
        <div className="max-w-7xl mx-auto animate-fade-in pb-12 sm:px-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8 items-stretch relative">
                ...
                <div className="lg:col-span-8">
                    {/* CẬP NHẬT: Thêm lg:min-h-[550px] xl:min-h-[600px] để card kéo dài hơn trên desktop */}
                    <div className="bg-sukem-card rounded-2xl shadow-sm border border-sukem-border p-5 md:p-6 h-full lg:min-h-[550px] xl:min-h-[550px]">
                        {/* CẬP NHẬT: Tăng khoảng cách lg:gap-10 */}
                        <div className="flex flex-col sm:flex-row items-start gap-6 md:gap-8 lg:gap-10 h-full">
                            {/* CẬP NHẬT: Tăng kích thước ảnh trên lg và xl để cân đối với card cao hơn */}
                            <div className="w-48 sm:w-56 md:w-64 lg:w-72 xl:w-80 flex-shrink-0 mx-auto sm:mx-0 sm:self-center shadow-md rounded-xl overflow-hidden border border-sukem-border relative group">
                            ...
                            <div className="flex-1 min-w-0 flex flex-col h-full self-stretch w-full">
                                ...
                                {/* CẬP NHẬT: Tăng kích cỡ chữ title trên Desktop cho phù hợp */}
                                <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold font-serif text-sukem-text leading-tight text-center sm:text-left mb-2">{story.title}</h1>

                                {story.alias && story.alias.length > 0 && (
                                    <p className="text-sm text-sukem-text-muted italic text-center sm:text-left mb-4">
                                ...
                                {/* --- STATS SECTION --- */}
                                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-x-3 gap-y-2 text-sukem-text-muted text-sm mb-5">
                                ...
                                {story.tags && (
                                    <div className="flex flex-wrap justify-center sm:justify-start gap-2 mb-6">
                                        {story.tags.map(tag => (
                                            <span key={tag} className="text-xs px-2.5 py-1 rounded-md bg-sukem-bg border border-sukem-border text-sukem-text hover:text-sukem-primary cursor-pointer transition-colors">
                                                #{tag}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                {/* Rating */}
                                <div className="mt-auto bg-sukem-bg/50 rounded-xl p-4 border border-sukem-border">
                                    <div className="flex items-center justify-between gap-3">
                                        <div className="flex items-center gap-4">
                                            <div className="flex flex-col items-center leading-none">
                                                <span className="text-2xl lg:text-3xl font-bold text-sukem-text">{story.rating.toFixed(1)}</span>
                                                <span className="text-[10px] lg:text-xs text-sukem-text-muted mt-1">{story.ratingsCount} đánh giá</span>
                                            </div>
                                            <div className="h-10 w-px bg-sukem-border"></div>
                                            <div className="flex flex-col gap-1">
                                                <span className="text-[10px] lg:text-xs uppercase font-bold text-sukem-text-muted">Đánh giá</span>
                                                <StarRating rating={story.rating} count={story.ratingsCount} userRating={userRating} onRate={handleRating} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {/* Buttons */}
                                <div className="flex flex-wrap gap-3 mt-5 md:flex-nowrap md:gap-3">
                                    {/* 1. NÚT ĐỌC */}
                                    {currentBookmark ? (
                                        <Link
                                            to={`/story/${story.id}/chapter/${currentBookmark.chapterId}`}
                                            className="btn-primary py-3 md:py-3 rounded-lg flex items-center justify-center gap-2 font-bold text-white bg-sukem-primary hover:bg-sukem-primary/90 transition-colors w-full md:w-auto md:flex-1 text-base active:scale-95 border border-sukem-primary"
                                        >
                                        ...
                                        <Link
                                            ...
                                            className={cn(
                                                "btn-primary py-3 md:py-3 rounded-lg flex items-center justify-center gap-2 font-bold text-white bg-sukem-primary hover:bg-sukem-primary/90 transition-colors w-full md:w-auto md:flex-1 text-base active:scale-95 border border-sukem-primary",
                                    ...
                                    {currentBookmark && (
                                        <button
                                            ...
                                            className="px-5 py-3 md:py-3 rounded-lg border border-sukem-border font-semibold text-sukem-text hover:bg-sukem-bg transition-colors flex-1 md:flex-none whitespace-nowrap"
                                        >
                                            Đọc lại
                                        </button>
                                    )}
                                    ...
                                    {currentUser && (
                                        <button
                                            ...
                                            className={cn(
                                                "px-4 md:px-4 py-3 md:py-0 rounded-lg border flex items-center justify-center transition-colors",
                                                isUserFavorite ? "border-red-200 bg-red-50 text-red-500" : "border-sukem-border text-sukem-text hover:text-red-500 hover:bg-sukem-bg"
                                            )}
                                            title="Yêu thích"
                                        >
                                    ...
                                    {currentUser?.role === 'admin' && (
                                        <button
                                            ...
                                            className="px-4 md:px-4 py-3 md:py-0 rounded-lg border border-sukem-border text-sukem-text hover:bg-sukem-bg flex items-center justify-center transition-colors"
                ...
                <div className="lg:col-span-4 relative">
                    <div className="bg-sukem-card rounded-2xl shadow-sm border border-sukem-border flex flex-col overflow-hidden max-h-[350px] lg:max-h-none lg:absolute lg:inset-0 lg:h-full">
                        <div className="p-4 lg:p-5 border-b border-sukem-border bg-sukem-bg/30 font-bold text-sukem-text flex items-center gap-2 shrink-0">
                            <InformationCircleIcon className="w-5 h-5 text-sukem-primary" />
                            Mô tả
                        </div>
                        <div className="p-5 overflow-y-auto custom-scrollbar flex-1">
                            <div className="text-sukem-text leading-loose text-justify text-sm lg:text-base whitespace-pre-wrap">
            ...
            {/* ... Giữ nguyên phần danh sách chương bên dưới ... */}
