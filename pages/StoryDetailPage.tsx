...
import { Story, Chapter } from '../types.ts'; // === THÊM Chapter VÀO IMPORT ===
import {
    PencilIcon, BookOpenIcon, HeartIcon, CalendarDaysIcon, EyeIcon, UserIcon, TagIcon,
    ListBulletIcon, MagnifyingGlassIcon, ArrowLeftIcon // Thêm icon
...
const StoryDetailPage: React.FC = () => {
  ...
  // === TÌM TÊN CHƯƠNG ĐANG ĐỌC ===
  const bookmarkedChapterTitle = useMemo(() => {
      if (!currentBookmark || !story) return null;
      const chapter = story.volumes
          .flatMap(v => v.chapters)
          .find(c => c.id === currentBookmark.chapterId);
  ...
  const handleRating = async (rating: number) => {
    if (!currentUser || !storyId) {
        // Thay thế alert bằng UI thân thiện hơn nếu có thời gian
    ...
    addRating(storyId, rating); // Update local context state
    try {
        await addRatingToStory(storyId, rating); // Call API
    } catch (e) {
        alert('Có lỗi xảy ra khi gửi đánh giá.');
        fetchStory(); // Revert UI on error
    }
  };
  ...
  const filteredVolumes = useMemo(() => {
    ...
    return story.volumes
        ...
        .filter(volume =>
            volume.title.toLowerCase().includes(lowerSearchTerm) || volume.chapters.length > 0
        );
    ...
    <div className="max-w-6xl mx-auto animate-fade-in space-y-8 md:space-y-12">
      ...
      <div className="bg-white dark:bg-stone-900 rounded-lg shadow-xl overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row gap-6 p-4 sm:p-6 md:gap-8 md:p-8">
          ...
          <div className="w-full md:w-1/3 flex-shrink-0">
            ...
            <div className="mt-6 flex flex-col gap-3">
              ...
              {currentBookmark ? (
                ...
                <div className="text-center p-3 bg-slate-100 dark:bg-stone-800 rounded-md text-slate-500 dark:text-stone-400 text-sm">Truyện chưa có chương.</div>
              ))}
              {/* Nút yêu thích */}
              {currentUser && (
                <button
                  onClick={() => toggleFavorite(storyId)}
                  // Cập nhật kiểu nút yêu thích
          ...
          {/* Thông tin chi tiết */}
          {/* Tăng khoảng cách các mục thông tin */}
          <div className="w-full md:w-2/3 space-y-6">
            ...
            {/* Thông tin tác giả, lượt xem, số chương */}
            {/* Căn chỉnh đẹp hơn */}
            ...
            {story.tags.length > 0 && (
                <div className="flex flex-wrap items-center gap-2 pt-0">
                    <TagIcon className="h-5 w-5 text-slate-400 dark:text-stone-500 mr-1"/>
                    {story.tags.map(tag => (
            ...
            {/* Mô tả */}
            {/* Bỏ max-h-48 và overflow-y-auto */}
            <div className="pt-2">
              <h2 className="text-xl font-semibold mb-3 font-serif text-slate-800 dark:text-slate-100">Mô tả</h2>
              <div className="pr-2 text-slate-700 dark:text-slate-300 leading-relaxed text-sm sm:text-base space-y-3">
                 {story.description ? (
                    // Tách đoạn dựa trên xuống dòng trong description
                    story.description.split('\n').map((paragraph, index) => (
                        <p key={index}>{paragraph || '\u00A0'}</p> // Thêm '\u00A0' để giữ khoảng cách nếu có dòng trống
                    ))
                 ) : (
        ...
        <div className="p-4 sm:p-6 md:p-8 border-t border-slate-200 dark:border-stone-800 bg-slate-50/50 dark:bg-stone-950/50">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-5 gap-3">
              ...
              <div className="relative w-full sm:w-64">
                <input
                    type="text"
                    placeholder="Tìm chương hoặc tên tập..." // Cập nhật placeholder
          ...
          <div className="space-y-4 max-h-[45rem] overflow-y-auto pr-3 -mr-1 custom-scrollbar">
              {filteredVolumes.length > 0 ? filteredVolumes.map(volume => (
                ...
                <div key={volume.id} className="bg-white dark:bg-stone-800/50 border border-slate-200 dark:border-stone-700/50 rounded-lg overflow-hidden shadow-sm">
                  ...
                  {/* Bỏ đường kẻ chia chương, dùng hover */}
                  <div>
                      {volume.chapters.map(chapter => {
                        ...
                        return (
                          <Link
                              ...
                              // Cập nhật class để làm nổi bật chương đang đọc rõ hơn
                              className={`block p-3 transition-colors duration-150 text-sm group border-t border-slate-100 dark:border-stone-700/50 first:border-t-0 ${
                                isReading
                                  ? 'bg-orange-100 dark:bg-amber-900/60 font-medium' // Nền đậm hơn
                          ...
                          >
                              <div className="flex justify-between items-center gap-4">
                                <div className="flex items-center gap-2 min-w-0">
                                  ...
                                  {/* Thêm padding left nếu không phải chương đang đọc */}
                      ...
                      {volume.chapters.length === 0 && chapterSearchTerm && (
                           <p className="text-sm p-4 text-center text-slate-500 dark:text-stone-500">Không tìm thấy chương nào khớp trong tập "{volume.title}".</p>
                      )}
                  </div>
                </div>
              )) : (
                 <p className="text-center py-10 text-slate-500 dark:text-stone-400">
