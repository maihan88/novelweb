...
const AdminDashboardPage: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const totalViews = safeStories.reduce((acc, story) => acc + story.views, 0);
    const totalChapters = safeStories.reduce((acc, story) => acc + (story.volumes?.reduce((volAcc: number, vol) => volAcc + vol.chapters.length, 0) || 0), 0);
    return {
                  ...
                  <tr key={story.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                      ...
                      <button 
                        ...
                        className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-200 p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700"
                        title="Xóa truyện"
                      >
