...
const SearchPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Cuộn lên đầu trang khi thay đổi trang hoặc kết quả tìm kiếm mới
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }, [query, status, chapterRange, currentPage]);
        ...
        <div className="mb-8 border-b border-sukem-border pb-6">
          ...
          <div className="mt-3 text-sukem-text-muted flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="flex items-center gap-2">
              {query && <span>Từ khóa: <span className="font-bold text-sukem-primary text-lg">"{query}"</span></span>}
              {query && <span className="hidden sm:inline">•</span>}
